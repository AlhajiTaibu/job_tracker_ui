    ...
    <section className="rounded-2xl border bg-card p-5 shadow-sm">
      ...
      {hasContacts ? (
        <div className="space-y-3">
          <div
            key={job_application.id || `${job_application.job_title}`}
          ...
          >
            <div className="mb-3">
              ...
              <p className="text-xs text-muted-foreground">
                {job_application.company_name?.trim() || "No company provided"}
              </p>
            </div>
...
export function ViewInterviewSheet({
  open,
  onOpenChange,
  interview,
}: ViewInterviewSheetProps) {
  const formattedDate =
    formatDate(interview?.created_at) ?? formatDate(interview?.date);
        ...
        <div className="flex min-h-full flex-col">
          ...
          <div className="space-y-5 px-6 py-6">
            <Section title="Overview" icon={<FileText className="h-4 w-4" />}>
              ...
              <InfoRow
                label="Job Title"
                value={interview.job_title}
                icon={<Briefcase className="h-4 w-4" />}
              />
              <InfoRow
                label="Round"
                value={String(interview.round)}
              ...
              <InfoRow
                ...
                value={interview.actual_duration}
                icon={<LucideTimer className="h-4 w-4" />}
              />
              <InfoRow
                label="Estimated Duration"
                value={interview.estimated_duration}
                icon={<LucideTimer className="h-4 w-4" />}
              />
              <InfoRow
                label="Created At"
            ...
            <RichSection
              ...
              value={interview.notes?.length ? interview.notes : null}
            />
          </div>
        </div>
      </SheetContent>
